...
import { getProportionalFontSize, isRTLCheck, RESPONSE_SUCCESS, TextFieldTypes } from '../utils/EDConstants';
...
export default class EDForgotPassword extends React.Component {
            ...
            <View style={styles.modalContainer} pointerEvents={this.state.isLoading ? 'none' : 'auto'}>
                ...
                <View style={[styles.modalSubContainer, {borderRadius:24}]}>
                    <EDRTLView style={{ alignItems: 'center', justifyContent: 'space-between' }}>
                        <EDRTLText title={strings("forgotPassword")} style={{
                        ...
                        <TouchableOpacity
                            ...
                            style={{ padding: 10, alignSelf: isRTLCheck() ? 'flex-start' : 'flex-end' }}>
                    ...
                    <EDThemeButton
                        ...
                        isRadius={true}
                        style={{margin:5 , width:'100%' , borderRadius:16 , height: Metrics.screenHeight * 0.075 }}
    ...
    onSubmitButtonHandler = () => {
        ...
        if (
            this.validationsHelper.validateEmail(this.state.phone.trim(),
                strings('emptyEmail')).trim().length > 0) {
            return;
        }
