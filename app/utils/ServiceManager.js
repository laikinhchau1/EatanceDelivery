...
* @param {Props of the screen from which the API is getting called} propsFromContainer
*/
export function displayPolyLine(userPolyLineParams, onSuccess, onFailure, propsFromContainer, googleMapsAPIKey) {
    ...
    let urlToCall = `https://maps.googleapis.com/maps/api/directions/json?origin=${[userPolyLineParams.destinationLatitude, userPolyLineParams.destinationLongitude]}&destination=${[userPolyLineParams.sourceLatitude, userPolyLineParams.sourceLongitude]}&key=${googleMapsAPIKey}&mode=driving&units=${DISTANCE_IN_MILES?"imperial":"metrics"}`;
