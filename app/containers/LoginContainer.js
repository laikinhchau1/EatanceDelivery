...
import { rememberLoginInRedux, saveCountryDataInRedux, saveLanguageInRedux, saveOnlineStatus, saveUserDetailsInRedux } from '../redux/actions/User'
import { saveLanguage, saveUserLoginDetails, saveUserStatus } from '../utils/AsyncStorageHelper'
import { showMailDialogue, showNoInternetAlert, showTopDialogue } from '../utils/EDAlert'
import { EDColors } from '../utils/EDColors'
import { debugLog, getProportionalFontSize, isRTLCheck, TextFieldTypes } from '../utils/EDConstants'
...
class LoginContainer extends React.Component {
    ...
    componentDidMount = () => {
        if (this.props.countryData !== undefined && this.props.countryData !== null && this.props.countryData.length == 0) {
                        ...
                        <View style={styles.textFieldStyle}>
                            ...
                            >
                                ...
                                {this.state.useEmail ?
                                    <EDRTLTextInput
                                        ...
                                        // containerStyle={{ marginTop: -this.state.height - 10 }}
                                        countryData={this.props.countryData}
                                        onCountrySelect={this.onCountrySelect}
                                        identifier={'email'}
                                        icon="email"
                            ...
                            <EDRTLView style={styles.buttonGroup}>
                                ...
                                <EDUnderlineButton
                                    ...
                                    textStyle={[styles.forgotPasswordText, { fontFamily: EDFonts.semiBold, fontSize: getProportionalFontSize(14), color: EDColors.black }]}
                            ...
                            {/* LOGIN BUTTON */}
                            <EDThemeButton
                                style={[styles.signInButton, { height: Metrics.screenHeight * 0.075, borderRadius: 16 }]}
                                label={strings('loginTitle')}
                                isLoading={this.state.isLoading}
                                onPress={this.buttonLoginPressed}
                                isRadius={true}
                                textStyle={{ fontFamily: EDFonts.semiBold, fontSize: getProportionalFontSize(16), }}
                            />
                            ...
                            <EDUnderlineButton
                                ...
                                label={this.state.useEmail ? strings("loginWithPhone") : strings("loginWithEmail")}
                            />
    ...
    toggleView = () => {
        ...
        this.setState({ useEmail: !this.state.useEmail, shouldPerformValidation: false })
        this.cardView.flipX()
    }
    toggleRemeber = () => {
    ...
    buttonLoginPressed = () => {
        ...
        if (
            ...
            this.validationsHelper.checkForEmpty(this.state.objLoginDetails.password.trim(),
                strings('emptyPassword')).length > 0
        ) {
            return
        } else {
            this.callLoginApi()
    ...
    navigateToHome = (dataToSave) => {
        ...
        saveUserStatus(dataToSave.availabilityStatus, onSuccess => { }, onFailure => { })
    ...
    onLoginUserFailure = onFailure => {
        ...
        if (onFailure.data.status == 2) {
            showMailDialogue(onFailure.data.message,
                [
        ...
        else { // showDialogue(onFailure.message, "", [],
            //     () => {
            //         this.state.objLoginDetails['phone'] = ''
            //         this.state.objLoginDetails['password'] = ''
            //         this.setState({ shouldPerformValidation: false })
